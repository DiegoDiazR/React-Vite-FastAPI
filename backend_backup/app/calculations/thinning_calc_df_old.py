...
class ThinningInputs:
    ...
    # Thicknesses
    t: float = 0.0                 # furnished thickness
    t_rdi: Optional[float] = None  # last known thickness (measured); if None => use t
    tc: float = 0.0                # cladding thickness (0 if none)
...
def compute_df_thinning(inp: ThinningInputs) -> Dict[str, Any]:
    ...
    Cr_bm = float(inp.Cr_bm or 0.0)
    Cr_cm = float(inp.Cr_cm or 0.0)
    ...
    out: Dict[str, Any] = {
        ...
        "step_3": {
            ...
            "age_tk_years": age_tk,
            "age_rc_years": age_rc,
            "age_rc_basis": "cladding Eq(2.10) if tc>0 and Cr_cm>0 else 0",
        },
        "step_5": {
            "A_rt": A_rt,
            "equation": "Eq(2.12)",
